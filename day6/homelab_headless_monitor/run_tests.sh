...
for f in system_health.py dashboard.py requirements.txt start.sh stop.sh Dockerfile templates/index.html; do
...
if ! curl -sf "$BASE_URL/api/metrics" -o /tmp/metrics.json 2>/dev/null; then
    ...
    if [ -n "$mem_p" ] && [ -n "$disk_p" ]; then echo "  OK metrics present"; else FAIL=1; fi
fi
